/**
 * esbuild bundler for the Component Bridge Figma plugin.
 *
 * Mirrors tooling/figma-plugin/build.mjs. Figma loads two artifacts: a sandbox
 * script (`dist/code.js`, has the `figma` API — pure TS, no React) and a single
 * self-contained UI document (`dist/ui.html`). The UI is a React app built on
 * the design-system component library, so we inline its JS + CSS into the HTML
 * template, leaving the manifest pointing at one file. Pass `--watch` to rebuild
 * on change.
 */
