      ...
      <div className={styles.appMain}>
          ...
          {STATS.map((s) => (
            <Card key={s.label} padding="md" className={styles.statCard}>
              ...
              <div className={styles.statFooter}>
                <Tag variant={s.variant} size="sm" emphasized={false}>
                  {s.delta}
                </Tag>
                <Meter
                  value={s.meter}
                  variant={s.variant === "error" ? "warning" : "default"}
                  size="sm"
                />
              </div>
            </Card>
        ...
        <div className={styles.dashGrid}>
          ...
          <Card padding="md" className={styles.activityCard}>
            <CardHeader className={styles.cardHeaderPlain}>
              <strong>Goals</strong>
            </CardHeader>
      ...
      <div className={styles.pageInner}>
        ...
        <div className={styles.pageHead}>
          ...
          <p className={styles.pageSubtitle}>
            Manage how your information appears across the workspace.
          </p>
        </div>
    ...
    <div className={styles.page}>
      <header className={styles.shopBar}>
        ...
        <div className={styles.shopBarActions}>
          <Select
            ...
            aria-label="Sort"
          />
          <Button intent="secondary" icon="tag">
            Cart{" "}
      ...
      <div className={styles.shopLayout}>
        <aside className={styles.filters}>
          ...
          />
          <Divider spacing="md" />
          <Button intent="primary" className={styles.filterApply}>
            Apply filters
          </Button>
        </aside>
          ...
          {PRODUCTS.map((p) => (
            <Card key={p.name} padding="none" className={styles.productCard}>
              <div
                ...
                style={{
                  background: `linear-gradient(135deg, hsl(${p.hue} 70% 55%), hsl(${p.hue + 30} 65% 42%))`,
                }}
              >
                {p.tag && (
                  <span className={styles.productTag}>
                    <Tag variant={p.variant} size="sm">
                      {p.tag}
                    </Tag>
                  </span>
                )}
...
const FEATURES = [
  {
    icon: "grid" as const,
    title: "Composable",
    text: "Every screen is built from the same tokenized primitives.",
  },
  {
    icon: "star" as const,
    title: "Accessible",
    text: "WCAG-checked color, focus, and keyboard support by default.",
  },
  {
    icon: "refresh" as const,
    title: "Themeable",
    text: "Re-skin the entire system from one source of truth.",
  },
];
...
const TIERS = [
  ...
  },
  {
    ...
    features: ["Unlimited projects", "Priority support", "Theme builder", "Figma sync"],
  ...
  },
  {
    ...
    features: ["SSO & roles", "Audit log", "Dedicated support"],
    intent: "secondary" as const,
    featured: false,
  },
];
    ...
    <div className={styles.page}>
      <nav className={styles.siteNav}>
        ...
        <div className={styles.siteLinks}>
          {/* Spans, not anchors: this is a mock site chrome inside the theme
              preview. Nothing here navigates, and `href="#"` would put three
              dead links in the tab order of a page that only shows colours. */}
          <span className={styles.siteLink}>Product</span>
          <span className={styles.siteLink}>Pricing</span>
          <span className={styles.siteLink}>Docs</span>
        </div>
      ...
      <section className={styles.hero}>
        <Tag variant="info" emphasized={false}>
          New · v2 design system
        </Tag>
        <h1 className={styles.heroTitle}>Design once. Ship everywhere.</h1>
        <p className={styles.heroText}>
          A token-driven component library that keeps product, brand, and code in perfect sync —
          ...
          {TIERS.map((t) => (
            ...
            >
              ...
              <ul className={styles.tierFeatures}>
                {t.features.map((feat) => (
                  <li key={feat}>
                    <Icon name="check" size={15} /> {feat}
                  </li>
                ))}
              </ul>
              <Button intent={t.intent} className={styles.tierButton}>
                Choose {t.name}
              </Button>
            </Card>
          ))}
      ...
      <footer className={styles.siteFooter}>
        ...
        <span className={styles.footerNote}>
          © 2026 Lattice Labs. Built with the design system.
        </span>
      </footer>
    </div>
  );
...
export function PreviewExamples() {
  ...
  switch (activeExample) {
    ...
    case "form":
      return <FormExample portal={portal} />;
    case "ecommerce":
      return <EcommerceExample portal={portal} />;
    case "marketing":
