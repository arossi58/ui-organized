...
**New release gate:** `examples/icon-smoke` is a minimal app using the documented setup and nothing else. The gate builds it for production, serves it, and counts the `<svg>` elements a browser actually renders. Nothing cheaper works — unit tests import the module by name, which defeats tree shaking; `vite dev` never reproduces it; and grepping the bundle proves nothing because production output is minified. It runs in CI and in `pnpm release`.
