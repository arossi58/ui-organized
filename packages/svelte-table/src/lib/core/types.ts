...
import type { ButtonProps } from "@ui-organized/svelte";
import type {
  BulkSelection,
  DataTableQuery,
  DetailState,
...
import type { SvelteTable, TableState } from "@tanstack/svelte-table";
import type { Virtualizer } from "@tanstack/svelte-virtual";

/**
 * Anything Svelte will render from a renderer function.
 *
 ...
 * `unknown` precisely so each adapter can narrow it to its own. A `Snippet` is
 * the Svelte 5 spelling of "a piece of markup you can hand around".
 */
export type SvelteNode = Snippet | string | number | null | undefined;

/** The Svelte column type. Identical to core's, with `meta.edit.render` narrowed. */
export type TableColumn<T extends RowData> = CoreTableColumn<T>;

export type TableEditRenderer<T extends RowData> = (ctx: TableEditContext<T>) => SvelteNode;
...
/**
 * ── How this API is reactive, and why it reads like plain values ────────────
 *
 * React's `DataTableApi` is a plain object rebuilt on every render, so its
 * fields are values. Vue's is a bag of refs, read with `.value`. Svelte's is
 * neither: every derived field is a **getter over a `$derived`**, so a component
 * writes `api.rows` and the read is tracked — the same property access it would
 * be in React, with none of Vue's `.value`.
 *
 * That is the second of the two places the plan predicted this port would come
 * out simpler, and it is why these interfaces name plain types rather than a
 * reactivity wrapper. The runes are on the other side of the getter.
 */
 ...
 *
 * Handed to `createDataTable` as a **getter** — `createDataTable(() => props)` —
 * rather than as an object. Svelte 5's `$props()` is reactive, so reading a field
 * off it inside a `$derived` tracks; passing the object itself would work too,
 * but the getter makes the intent explicit and lets a caller compose options
 * from several sources without building a proxy.
 */
...
export interface DataTableApi<T extends RowData> {
   ...
   *
   * The *Svelte* instance, so its rune-aware atoms come with it. The instance
   * is created once and mutated in place; its reactivity lives in the atoms it
   * holds, which track when read inside a `$derived` or a template.
   */
  table: SvelteTable<UioTableFeatures, T>;
  /**
   * The controlled state — sorting, pagination, visibility, selection.
   *
   * React's adapter hangs a `state` getter off the table instance; Svelte's
   * exposes rune-aware `atoms` instead. It is handed back here rather than
   ...
   */
  readonly state: Partial<TableState<UioTableFeatures>>;
  ...
  /**
   * Callback refs, the way React takes them rather than the way Vue does.
   *
   * Svelte 5 has no `ref` object to bind: `bind:this` assigns to a variable, and
   * a variable assignment is not something a `$derived` elsewhere can watch. A
   * callback that writes into the api's own `$state` is what makes the node's
   * arrival reactive, which is what the resize and scroll observers need.
   */
