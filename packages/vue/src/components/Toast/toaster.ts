 ...
 * shape the React and Svelte packages have, and the reason `useToastManager()`
 * returns a static object rather than holding any state.
 *
 ...
 * Kept identical to the other libraries so the same code reads the same way.
 */
const manager = {
 ...
 *
 * Named `useToastManager` to match the other libraries, though it is not a
 * composable and can be called anywhere — there is no state behind it.
 */
