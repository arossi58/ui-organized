...
export interface DatePickerProps {
   ...
   * the conversion happens at this boundary. Use `v-model` for two-way binding.
   */
  modelValue?: string[];
  ...
  locale?: string;
  /** Open state of the calendar. Use `v-model:open` for two-way binding. */
  open?: boolean;
  ...
  name?: string;
  /**
   * DOM element to teleport the calendar surface into. Defaults to `body`.
   *
   * React spells this `container` and takes a ref object, because that is what
   * its `<Portal>` wants. Teleport takes the element, so it is named the way
