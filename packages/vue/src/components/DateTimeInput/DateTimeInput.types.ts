...
/**
 * Props for DateTimeInput. `type` is fixed to `"datetime-local"`; every other
 * native input attribute (`name`, `step`, `placeholder`, …) falls through
 * `$attrs` onto the control.
 *
...
export interface DateTimeInputProps {
  ...
  disabled?: boolean;
  /** The field's ISO value (`YYYY-MM-DDTHH:mm`). Use `v-model` for two-way binding. */
  modelValue?: string;
  /** Initial ISO value for uncontrolled usage. */
  defaultValue?: string;
  /** Earliest selectable date (ISO `YYYY-MM-DDTHH:mm`). */
  min?: string;
  /** Latest selectable date (ISO `YYYY-MM-DDTHH:mm`). */
  max?: string;
  /**
