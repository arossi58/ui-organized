...
describe("canonical typography exports", () => {
  ...
  it("exposes sizes + leadings as plain px numbers", () => {
    ...
    expect(typeLeadingTokens["body-medium"]).toBe(20);
    expect(typeSizeTokens["display-xlarge"]).toBe(64);
    expect(typeLeadingTokens["display-xlarge"]).toBe(72);
  ...
  it("exposes fonts and weights", () => {
    expect(typeFontTokens.heading).toBe("Roboto");
    expect(typeFontTokens.body).toBe("Roboto");
