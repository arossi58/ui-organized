...
describe("Button", () => {
    it("renders a native button with the default variant classes", () => {
        ...
        expect([...button.classList].sort()).toEqual(["btn", "btn--primary", "btn--md", "text-default-body-large"].sort());
    });
    it("agrees with the shared recipe for every intent and size", () => {
        const intents = ["primary", "secondary", "tertiary", "ghost", "destructive", "destructive-ghost"];
