...
/**
 * Props for DateTimeInput. Native datetime-local props (`min`, `max`, `step`,
 * `name`, `oninput`, …) are inherited; `type` is fixed to `"datetime-local"`.
 *
...
export interface DateTimeInputProps
  ...
  size?: ControlSize;
  /** The field's ISO value (`YYYY-MM-DDTHH:mm`). Bindable: `bind:value`. */
  value?: string;
