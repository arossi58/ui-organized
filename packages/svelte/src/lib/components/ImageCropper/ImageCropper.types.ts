 ...
 *
 * `svelte-package` emits no `.d.ts` for a component whose props interface
 * reaches a type it cannot re-export, so a private base interface silently costs
 * this component its types. Every prop is spelled here for that reason — and
 * `CropRect` is exported for the same one: `onCropChange`'s argument has to be
...
export interface ImageCropperProps {
  ...
  /** Controlled zoom level. Bindable: `bind:zoom`. */
  zoom?: number;
