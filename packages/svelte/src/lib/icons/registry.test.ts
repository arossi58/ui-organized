...
const ICON_PACKAGES = ["@lucide/svelte", "@tabler/icons-svelte"];

/** Modules allowed to import an icon library — one per library, each a subpath entry. */
const ADAPTERS = new Set(["src/lib/icons/lucide.ts", "src/lib/icons/tabler.ts"]);

/**
 * The libraries with an adapter. Heroicons is deliberately absent — no
 * maintained Heroicons-as-Svelte-components package exists; see the note on
 * `PACKAGE_FOR` in components/Icon/warnMissingIconSet.ts.
 */
const LIBRARIES = ["lucide", "tabler"];

function sourceFiles(dir = "src/lib"): string[] {
  const out: string[] = [];
 ...
 *
 * Comments are dropped first. `registry.ts` documents the directly-supplied
 * component escape hatch with an example import from `@lucide/svelte`, and
 * without this the guard reads that prose as the main entry importing Lucide.
 * Whole-line `//` only, so a URL inside a string survives.
 */
...
function resolveLocal(from: string, spec: string): string | null {
  ...
  for (const ext of ["", ".ts", ".svelte", "/index.ts"]) {
...
describe("icon library isolation", () => {
  it("keeps every icon library out of the main entry's module graph", () => {
    const reachable = reachableFrom("src/lib/index.ts");
    ...
    expect(offenders, "src/lib/index.ts must not reach any icon library").toEqual([]);
  });
  ...
  it("confines each icon library to exactly one adapter module", () => {
    ...
    // Both are still adapted — this would otherwise pass by deleting them.
  ...
  it("exposes one subpath entry per library, each registering itself", () => {
    for (const lib of LIBRARIES) {
      const src = readFileSync(`src/lib/icons/entry-${lib}.ts`, "utf8");
  ...
  it("declares every subpath entry in package.json exports", () => {
    ...
    for (const lib of LIBRARIES) {
      ...
      expect(entry!.svelte).toBe(`./dist/icons/entry-${lib}.js`);
    }
  });
  ...
  it("keeps the registry on a global key so a duplicated module still shares state", () => {
    ...
    const src = readFileSync("src/lib/icons/registry.ts", "utf8");
